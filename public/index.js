...
const loadPage = () => {
  ...
  switch (currentPage) {
    ...
    case 'addFlower':
      if (USER_NAME) {
        $("#root").load(`${BASE_URL}addFlower?user=${USER_NAME}`, function () {
        });
      } else {
        $("#root").load(`${BASE_URL}about`);
      }
...
const loadCatalog = async () => {
  ...
  let html = "";
  let html2 = "";
  USER_TYPE == "supplier" ? html2 += `
    <button<li><a onclick="goToPage('addFlower');" >ADD
    FLOWER</a></li></button>
    ` : ``
  ...
  document.getElementById("flowerlist").innerHTML =html2+ html;
...
const loadUsers = async (_add = false) => {
  ...
  if (_add) {
    ...
    <td><select class="form-control" name="user_type" id="user_type_adduser">
        ...
        ${USER_TYPE == "manager" ? `<option value="supplier">supplier</option>` : ``}
  ...
  users.map(user => html += `
   <tr id="tr1_${user._id}">
    ...
    <td >${user.category}</td>
    <td>  
        <a class="edit" title="Edit" onclick="toggleEdit('${user._id}');"><i class="material-icons">&#xE254;</i></a>
        ${USER_TYPE == "manager"
      ? `<a class="delete" title="Delete" onclick="deleteUser('${user._id}');"><i class="material-icons">&#xE872;</i></a>`
      : ""}
    </td>
   </tr>
   
   <tr id="tr2_${user._id}" class="edit-tr">
    <td><input type="text" class="form-control" value="${user.name}" id="name_${user._id}"></td>
    <td><input type="text" class="form-control" value="${user.mail}" id="mail_${user._id}"></td>
    <td><input type="text" class="form-control" value="${user.username}" id="username_${user._id}"></td>
    ${USER_TYPE == "manager"
      ? `<td><input type="text" class="form-control" value="${user.password}" id="password_${user._id}"></td>`
      : `<td><input type="text" class="form-control" value="Unauthorized"></input></td>`}
    <td><input type="text" class="form-control" value="${user.phone}" id="phone_${user._id}"></td>
    <td><select class="form-control" value="${user.category}" id="user_type_${user._id}">
        <option value="customer">customer</option>
        ${USER_TYPE == "manager" ? `<option value="supplier">supplier</option>` : ``}
        ${USER_TYPE == "manager" ? `<option value="employee">employee</option>` : ``}
        ${USER_TYPE == "manager" ? `<option value="manager">manager</option>` : ``}
    ...
    <td>
        <a class="add" title="Add" onclick="updateUser('${user._id}');" ><i class="material-icons">&#xE03B;</i></a>
    </td>
...
const saveFlower = async () => {
  ...
  //let image = $('#phone_adduser').val();

  let body = { name, color, price, description };
